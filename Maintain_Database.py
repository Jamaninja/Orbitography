...
def createDatabase(path):
    ...
    for i in data:
        ...
        n += 1
        if not n % 50:
            print(f"Creating database: {100*n/length:00.2f}%")
        print("Saving database.")

    df.to_csv(f"{path["directory"]}/{path["database"]}")
    return path

def Update_Database():
    mtime = datetime.fromtimestamp(os.path.getmtime(f"{path["directory"]}/{path["database"]}"))
    time_range = f"{Datetime_to_StarTrack(mtime)}--now"
    

def Datetime_to_StarTrack(dt):
    return f"{dt.strftime("%Y-%m-%d")}%20{dt.strftime("%H:%M:%S")}"

path = {
    "directory":    "Ephemeris_Database",
    "ephemeris":    "30_Day_Ephemeris.json",
